...
module.exports = (client) => {
    ...
    client.on("guildUpdate", async function (oldGuild, newGuild) {
        ...
        if (oldGuild.explicitContentFilter !== newGuild.explicitContentFilter) {
            const embed = new Discord.MessageEmbed()
                ...
                .setColor(updateColor || 'BLUE')
                .setFooter(footerMsg, newGuild.iconURL())
                .addField('Old Filter:', `\`\`\`${oldGuild.explicitContentFilter}\`\`\``, false)
                .addField('New Filter:', `\`\`\`${newGuild.explicitContentFilter}\`\`\``, false)
                .addField('Executor:', `\`\`\`${audit.executor.username}\`\`\``, true)
                .setTimestamp();
    ...
    client.on("voiceStateUpdate", async (oldState, newState) => {
        ...
        if (!oldState.streaming && newState.streaming) {
            const embed = new Discord.MessageEmbed()
                .setTitle(':microphone2:  〔 Member Stream ON 〕')
    ...
    client.on("inviteDelete", async (invite) => {
        console.log('in')
        const audit = await invite.guild.fetchAuditLogs({ type: "INVITE_DELETE" }).then(log => log.entries.first());
        let logchannel = db.fetch(`logs_${invite.guild.id}`);
        if (!logchannel) return;
